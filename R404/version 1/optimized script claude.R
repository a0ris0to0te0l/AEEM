...
# FIXED: Proper UCV with global fallback implementation
# ==============================================================================
...
get_bw <- function(rule, x, fallback = TRUE, verbose = FALSE) {
  ...
  bw <- tryCatch({
             ...
             suppressWarnings({
               ...
               ucv_result <- tryCatch({
                 ...
                 if (grepl("occurred at one end", conditionMessage(w))) {
                   if (verbose) message("      UCV (native): Boundary minimum detected")
               ...
               # FALLBACK: Use manual implementation if native failed
               if (is.na(ucv_result) || !is.finite(ucv_result)) {
                 if (verbose) message("      UCV: Trying manual fallback implementation...")
                 
                 ucv_result <- manual_bw_ucv(x, lower, upper)
                 
                 if (is.na(ucv_result) || !is.finite(ucv_result)) {
                   if (verbose) message("      UCV (manual): Failed, using nrd0")
                   return(fallback_bw)
                 }
                 
                 if (verbose) message("      UCV (manual): Success")
               } else {
                 if (verbose) message("      UCV (native): Success")
               }
    ...
    if (fallback) fallback_bw else NA_real_
  })
  
  # Final validation
  if (is.na(bw) || !is.finite(bw) || bw <= .Machine$double.eps * 100) {
    return(if (fallback) fallback_bw else NA_real_)
  }
...
compute_density_df <- function(xvec, kernel, bw, x_grid = NULL) {
  ...
  # Fallback to alternative bandwidths
  if (is.character(bw)) {
...
smooth_loess <- function(x, y, bw_numeric) {
  tryCatch({
    ...
    # Compute span with minimum neighbors
...
generate_histograms <- function(data, bins, kernels, bw_rules, out_dir, 
  ...
  for (bn in names(bins)) {
    ...
    for (k in kernels) {
      for (b in bw_rules) {
        dens <- compute_density_df(data$pct, k, b)
        
        ttl <- sprintf("%s Distribution Analysis", period_name)
        subt <- sprintf("Bins: %d | Kernel: %s | Bandwidth: %s", nbins, 
                        str_to_title(k), toupper(b))
        
        fname <- file.path(out_dir, sprintf("hist_%s_%s_%s.pdf", bn, k, safe_bw_label(b)))
    ...
    for (b in bw_rules) {
      dlist <- compact(lapply(kernels, function(k) {
        d <- compute_density_df(data$pct, k, b)
    ...
    # Kernel overlays
    for (k in kernels) {
      dlist <- compact(lapply(bw_rules, function(b) {
        d <- compute_density_df(data$pct, k, b)
