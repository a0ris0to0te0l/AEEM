...
#
# Data: Eurostat API ({eurostat} R package); World Bank API ({wbstats}) as fallback
# Author: [Your name]
# Date:   Sys.Date()
################################################################################
...
# Kosovo (XK) and Serbia (RS) have limited Eurostat coverage; a World Bank
# fallback is attempted for those.
...
# ISO-3 codes for World Bank fallback
WB_ISO3 <- c(
  AL = "ALB", BA = "BIH", BG = "BGR", EL = "GRC", HR = "HRV",
  ME = "MNE", MK = "MKD", RO = "ROU", RS = "SRB", SI = "SVN",
  XK = "XKX"
)
...
# --------------------------------------------------------------------------
# World Bank fallback for countries with gaps (annual → interpolated quarterly)
# --------------------------------------------------------------------------

wb_fallback <- function(iso3_codes) {
  cat("  Attempting World Bank fallback for:", paste(iso3_codes, collapse = ", "), "\n")
  tryCatch({
    raw <- wb_data(
      indicator = c(
        gdp    = "NY.GDP.MKTP.KD",        # GDP constant 2015 USD
        gov_exp = "NE.CON.GOVT.KD",        # Government consumption constant 2015 USD
      ...
      rename(country_wb = iso3c, year = date) %>%
      # Reverse-map ISO3 → Eurostat code
      mutate(country = names(WB_ISO3)[match(country_wb, WB_ISO3)])
    raw
  }, error = function(e) {
    warning("World Bank fallback also failed: ", e$message)
    NULL
  })
}

# Identify countries with insufficient Eurostat quarterly data
MIN_OBS <- 24  # Need at least 6 years of quarters

eurostat_coverage <- if (!is.null(gdp_raw)) {
  gdp_raw %>%
    filter(na_item == "B1GQ", !is.na(values)) %>%
    count(geo) %>%
    filter(n >= MIN_OBS) %>%
    pull(geo)
} else character(0)

countries_needing_wb <- setdiff(COUNTRIES, eurostat_coverage)
if (length(countries_needing_wb) > 0) {
  cat(sprintf(
    "\nCountries with insufficient Eurostat coverage (< %d obs): %s\n",
    MIN_OBS, paste(countries_needing_wb, collapse = ", ")
  ))
  wb_data_raw <- wb_fallback(WB_ISO3[countries_needing_wb])
} else {
  wb_data_raw <- NULL
}
...
panel_eurostat <- if (!is.null(gdp_raw) && !is.null(unemp_raw)) {
  clean_eurostat(gdp_raw, unemp_raw)
} else {
  data.frame()
}

# Interpolate annual WB data to quarterly (linear) and merge
clean_wb <- function(wb_raw) {
  if (is.null(wb_raw) || nrow(wb_raw) == 0) return(data.frame())
  wb_raw %>%
    filter(!is.na(country), !is.na(gdp), !is.na(gov_exp)) %>%
    group_by(country) %>%
    arrange(year) %>%
    # Expand to quarterly by repeating annual values (simple interpolation)
    reframe({
      yr      <- year
      q_dates <- do.call(c, lapply(yr, function(y) {
        as.Date(paste0(y, c("-01-01", "-04-01", "-07-01", "-10-01")))
      }))
      n_q <- length(q_dates)
      # Linear interpolation for GDP and gov_exp
      approx_gdp <- approx(as.numeric(as.Date(paste0(yr, "-01-01"))),
                           gdp, xout = as.numeric(q_dates))$y
      approx_gov <- approx(as.numeric(as.Date(paste0(yr, "-01-01"))),
                           gov_exp, xout = as.numeric(q_dates))$y
      approx_une <- approx(as.numeric(as.Date(paste0(yr, "-01-01"))),
                           unemp, xout = as.numeric(q_dates))$y
      data.frame(date = q_dates, gdp = approx_gdp,
                 gov_exp = approx_gov, unemp = approx_une)
    }) %>%
    filter(!is.na(gdp), !is.na(gov_exp)) %>%
    ungroup()
}

panel_wb <- clean_wb(wb_data_raw)

# Combine, preferring Eurostat where available
panel_raw <- bind_rows(
  panel_eurostat %>% mutate(source = "Eurostat"),
  panel_wb       %>% mutate(source = "WorldBank")
) %>%
  group_by(country, date) %>%
  # Keep Eurostat if duplicated
  arrange(source) %>%
  slice(1) %>%
  ungroup()

cat(sprintf("  Combined panel: %d observations across %d countries.\n",
...
run_adf <- function(x, name, country) {
  x <- x[!is.na(x)]
...
adf_table <- panel_est %>%
  ...
  group_modify(function(d, k) {
    bind_rows(
      run_adf(d$ln_gdp, "ln_gdp", k$country),
      run_adf(d$ln_g,   "ln_g",   k$country),
      run_adf(d$dy,     "dy",     k$country),
      run_adf(d$dg,     "dg",     k$country)
    )
  }) %>%
  ungroup()

print(adf_table, n = Inf)
...
run_lp_iv <- function(data, h, cluster_se = FALSE) {
  ...
  tryCatch({
    ...
    list(
      ...
      pval_slack    = ct["cum_g_slack", "Pr(>|z|)"],
      pval_exp      = ct["cum_g_exp",   "Pr(>|z|)"],
...
extract_irf <- function(lp_list, label) {
  if (length(lp_list) == 0) return(NULL)
...
irf_countries <- do.call(rbind, lapply(USABLE_COUNTRIES, function(cc) {
...
# Multiplier table
compute_multipliers <- function(irf_df, horizons = c(4, 8, 12)) {
  irf_df %>%
    group_by(group) %>%
    summarise(
      across(
        .cols = c(beta_slack, beta_exp),
        .fns  = setNames(
          lapply(horizons, function(hh) {
            function(x) x[which(irf_df$horizon[irf_df$group == cur_group()] == hh)]
          }),
          paste0("h", horizons)
        )
      ),
...
make_mult_table <- function(irf_df, horizons = c(4, 8, 12)) {
  do.call(rbind, lapply(unique(irf_df$group), function(grp) {
    sub <- irf_df %>% filter(group == grp)
    rows <- lapply(horizons, function(hh) {
      r <- sub %>% filter(horizon == hh)
